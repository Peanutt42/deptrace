...
impl DependencyConfig {
    /// TODO: this should not be in config, but in the actual `Dependency`
    pub fn provides_shared_library(&self, shared_lib_name: &str) -> bool {
        self.provides.contains(&shared_lib_name.to_string())
            || self
                .subdependencies
                .iter()
                .any(|subdep| subdep.provides_shared_library(shared_lib_name))
    }
}
...
impl DependencyNameOrDependencyConfig {
    ...
    pub fn provides_shared_library(&self, shared_lib_name: &str) -> bool {
        match self {
            Self::Name(_) => false,
            Self::Config(config) => config.provides_shared_library(shared_lib_name),
        }
    }
}
